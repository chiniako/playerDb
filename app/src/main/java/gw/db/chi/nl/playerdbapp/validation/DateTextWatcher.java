package gw.db.chi.nl.playerdbapp.validation;
...
public class DateTextWatcher implements TextWatcher {
    ...
    public static final int MAX_FORMAT_WITH_SEP_LENGTH = 10;
    ...
    public void onTextChanged(CharSequence text, int start, int before, int count) {
        ...
        if (digitLen < MIN_FORMAT_LENGTH) {
            updatedText = digitsOnly;
            return;
        }
        if (digitLen > MAX_FORMAT_LENGTH) {
            updatedText = text.subSequence(0, MAX_FORMAT_WITH_SEP_LENGTH).toString();
            return;
        }
